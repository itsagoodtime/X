...
local settingsTable = {
	General = {
		...
		rayfieldOpen = {Type = 'bind', Value = 'K', Name = '隐藏脚本按键(手机玩家不用管)'},
		-- buildwarnings
		-- rayfieldprompts

	},
	System = {
		usageAnalytics = {Type = 'toggle', Value = true, Name = '匿名情况分析'},
	}
}
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						...
						RayfieldLibrary:Notify({Title = "密钥系统", Content = "密钥自动保存成功.", Image = 3605522284})
					end
				else
...
Main.Search.Input:GetPropertyChangedSignal('Text'):Connect(function()
	if #Main.Search.Input.Text > 0 then
		if not Elements.UIPageLayout.CurrentPage:FindFirstChild('SearchTitle-fsefsefesfsefesfesfThanks') then 
			...
			searchTitle.Title.Text = "搜寻'"..Elements.UIPageLayout.CurrentPage.Name.."'的结果"
